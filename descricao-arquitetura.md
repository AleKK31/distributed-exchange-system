...
Responsável pelo gerenciamento dos itens cadastrados para troca. Recebe requisições do NGINX, persiste os dados no banco e publica eventos no **RabbitMQ** via AMQP. Também consome os eventos `match.*` para atualizar o status dos itens conforme a proposta avança (*"negociando"*, *"trocado"* ou de volta a *"disponível"*).

### Serviço de Match
Possui a regra de negócio do sistema de trocas. Funciona como um **worker** que consome eventos do RabbitMQ e também expõe endpoints HTTP para os usuários aceitarem ou recusarem propostas. Ao receber uma nova publicação, busca no banco de dados um item compatível (onde o que o usuário X oferece é o que o usuário Y deseja, e vice-versa). Após encontrar um par, publica os eventos de resultado de volta no RabbitMQ. Também reage à atualização e à remoção de publicações, refazendo a busca de par ou cancelando a proposta pendente afetada.
...
Após receber o evento `publicacao.criada`, o Sv. Match consulta o banco em busca de um item onde o `item_oferto` seja igual ao `item_desejado` do novo item, e vice-versa. Ao encontrar um par, publica `match.encontrado` no RabbitMQ. O Sv. Notificação consome esse evento e envia via WebSocket para os dois usuários envolvidos. A edição de uma publicação (`publicacao.atualizada`) dispara a mesma busca, formando um par quando os itens passam a ser compatíveis.
...
### Fluxo de Cancelamento por Remoção
Se uma publicação que faz parte de uma proposta pendente é removida, o Sv. Publicações emite `publicacao.removida`. O Sv. Match cancela a proposta e publica `match.cancelado`. A publicação remanescente volta ao estado *"disponível"* para novo matching, enquanto a removida permanece *"removida"*; o Sv. Notificação avisa os usuários envolvidos.
